...
export function buildRollouts(
  ...
  for (const [topic, entries] of byTopic) {
    for (const entry of entries) {
      ...
      //
      // "A must move before B stops" is the right sentence only while B has not stopped. Where the
      // far side no longer declares the baseline, it stopped already: the deadline has passed, the
      // gap is live, and a future-tense sentence reads to someone under pressure as "not yet
      // urgent". An on-call engineer read exactly that off `inventory:reserve` while the call was
      // failing 100%.
      //
      // Checked on BOTH arms, and on disjointness. The first version of this tested only the
      // catch-up arm, so the mirror image — an adapter that has left the baseline while the owner is
      // still on it — kept the calmest label in the vocabulary. A platform engineer constructed that
      // case and got a card whose headline said "can move whenever it is ready" directly above a
      // sub-line saying nothing in the estate handles anything being sent. Two sentences on one
      // card, generated by independent paths, with the severe one in supporting type.
      const ownerLeftBaseline = ownerAtBaseline.length === 0;
      const adapterLeftBaseline = adapterAtBaseline.length === 0;
      const breached = disjoint
        || (state === 'awaitingAdapter' && ownerLeftBaseline)
        || (state === 'awaitingOwner' && adapterLeftBaseline);

      let constraint: string | null = null;
      if (ownerSide !== 'mixed' && state === 'awaitingOwner' && breached) {
        constraint = `${list(adapterAtCurrent)} no longer ${adapterVerb.third} ${topic} `
          + `${baselineVersion}, and ${list(outstandingOwners)} is still on it. `
          + `${list(outstandingOwners)} must ${ownerVerb.imperative} ${entry.version}.`;
      } else if (ownerSide !== 'mixed' && state === 'awaitingAdapter' && ownerLeftBaseline) {
